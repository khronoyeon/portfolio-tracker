...
CODES_DB_ID = "39f2e440054081fd9bb4d60689f92406"      # 🔐 접속 코드 (관리자 전용 페이지)
LOG_DB_ID = "39f2e44005408141af78e50d7ff32db6"        # 🕐 접속 기록 (관리자 전용 페이지)
...
# 수집 주기: 업로드 후 이 일수 이내(신작)만 매일 수집, 그 외는 주 1회(월요일)만 수집
OLD_AFTER_DAYS = 7
...
def main():
    ...
    for page in pages:
        ...
        code = instagram_shortcode(link) if not vid else None
        if not vid and not code:
            skipped += 1
            print("  링크 형식을 인식하지 못함:", link)
            continue
        # 업로드 7일 지난 영상은 월요일에만 수집
    ...
    if rested:
        print("업로드 {}일 지난 영상 {}개는 월요일에만 수집".format(OLD_AFTER_DAYS, rested))
